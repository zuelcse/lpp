...
if (!function_exists('numberToBanglaWords')) {
    function numberToBanglaWords($number)
    {
        ...
        if ($number < 100) {
            ...
            return $bn[$hundreds] . ' শত' . ($remainder ? ' ' . numberToBanglaWords($remainder) : '');
        } elseif ($number < 100000) {
            $thousands = (int)($number / 1000);
            $remainder = $number % 1000;
            return numberToBanglaWords($thousands) . ' হাজার' . ($remainder ? ' ' . numberToBanglaWords($remainder) : '');
        } elseif ($number < 10000000) {
            $lakh = (int)($number / 100000);
            $remainder = $number % 100000;
            return numberToBanglaWords($lakh) . ' লাখ' . ($remainder ? ' ' . numberToBanglaWords($remainder) : '');
        } else {
            $crore = (int)($number / 10000000);
            $remainder = $number % 10000000;
            return numberToBanglaWords($crore) . ' কোটি' . ($remainder ? ' ' . numberToBanglaWords($remainder) : '');
        }
    }
}

if (!function_exists('numberToBangla')) {

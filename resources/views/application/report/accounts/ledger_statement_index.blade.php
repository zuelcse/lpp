...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <form method="POST" target="_blank" action="{{route('report-accounts-ledger-statement')}}" enctype="multipart/form-data" onsubmit="return reportRestriction()">
                    @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-xxl">
                                                ...
                                                <div class="col-9 col-sm-9">
                                                    <select type="text" class="form-control form-control-sm js-example-basic-single" id="ledger" name="ledger">
                                                        ...
                                                        @foreach ($ledger as $key => $data)
                                                            <option value="{{ $data->id }}">{{ $data->alias.'-' .$data->name . ($data->mobile?'-'.$data->mobile:'') }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>

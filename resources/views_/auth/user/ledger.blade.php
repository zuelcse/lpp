...
@section('title', 'Modify Ledger group Permission Allocation')
@section('content')
...
<div class="card">
   ...
   <form action="{{ route('storeUpdateUserLadger') }}" method="POST">
      @csrf
      ...
      <div class="col-lg-12 col-md-12 mb-12 grid-item">
                        ...
                        <table class="table" class="font-size: 0.7em">
                            <thead>
                                <tr>
                                    ...
                                    <th>Ledger Parents</th>
                                </tr>
                            </thead>
                            <tbody class="table-border-bottom-0">  
                                @foreach($ledger as $item)
                                    <tr>
                                        <td><input type="checkbox" name="ledger_group[]" value="{{$item->parent}}" class="mr-1" @if(in_array($item->parent,$ledgerPermission,true)) checked @endif></td>
                                        <td>{{ $item->parent }}</td>
                                    </tr>
                                @endforeach
        ...
        @can('edit_user')
            <button type="submit" class="btn btn-primary font-weight-bolder float-right mt-5" onclick="return confirm('Are you sure you want to modify ledger assignments?')">
               <i class="far fa-check-circle"></i>Modify ledger group allocation
            </button>  
        @endcan
